    ...
    func updateTipTotals(){
        ...
        if numberOfPeople != nil {
            if numberOfPeople! > 0 {
                ...
                let alert = UIAlertController(title: "Warning", message: "Invalid number of people...", preferredStyle: UIAlertControllerStyle.Alert)
                let cancelAction:UIAlertAction=UIAlertAction(title: "Cancel", style:UIAlertActionStyle.Cancel, handler: nil)
                alert.addAction(cancelAction)
                let okAction:UIAlertAction=UIAlertAction(title: "OK", style: UIAlertActionStyle.Default, handler: { action in self.people.text="1"
