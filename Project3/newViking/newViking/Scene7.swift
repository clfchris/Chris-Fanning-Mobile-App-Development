...
class Scene7: SKScene {
    ...
    override func didMoveToView(view: SKView) {
        ...
        myLabel2.text = "AND HE SCARED IT AWAY, NEVER TO RETURN.";
    ...
    override func touchesBegan(touches: NSSet, withEvent event: UIEvent) {
        ...
        for touch: AnyObject in touches {
            ...
            let reverseScene = Scene6(size: CGSize(width: w, height: h))
            ...
            if playButton.containsPoint(location){
                let moveRight = SKAction.moveByX(100.0, y: 1.0, duration: 0.1)
                let moveLeft = SKAction.moveByX(-1000.0, y: -1.0, duration: 0.5)
                let moveBack = SKAction.moveByX(1000.0, y: 0.0, duration: 1.0)
                button.runAction(SKAction.sequence([moveRight, moveLeft, moveBack]))
                dragonSky.runAction(SKAction.sequence([moveLeft]))
            }
        }
    }
